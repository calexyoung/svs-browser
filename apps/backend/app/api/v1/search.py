...
class BrowseSortOption(str, Enum):
    """Sort options for browse (no relevance since there's no query)."""
    DATE_DESC = "date_desc"
    DATE_ASC = "date_asc"
...
async def browse(
    ...
    sort: BrowseSortOption = Query(BrowseSortOption.DATE_DESC, description="Sort order"),
    limit: int = Query(20, ge=1, le=100, description="Results per page"),
    offset: int = Query(0, ge=0, description="Pagination offset"),
    db: AsyncSession = Depends(get_db),
) -> SearchResponse:
    """
    Browse all SVS visualizations without a search query.
    ...
    Unlike /search, this endpoint does not require a query and returns all pages.
    """
